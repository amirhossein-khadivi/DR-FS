...
func_ML_incremental_feature_subsets <- function(freq_rank_index, feature_beni, feature_mali, num_iteration, num_top_feature) {
  ...
  for (i in 1:num_iteration) {
    ...
    for (j in 1:num_top_feature) {
      ...
      metric_svm_freqy[i, j, ] <- utsw_machine_learning_classifiers_train_test(tmp_Xtrain, Ytrain, tmp_Xtest, Ytest, 'lsvm')
    }
